...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($id) || empty($tanggal_kunjungan) || empty($pilihan_paket_wisata) || empty($jenis_wisatawan) || empty($nama) || $pax <= 0) {
        ...
        header("Location: ../editpengunjung.php");
        exit;
    }
    ...
    if ($jenis_wisatawan == 'Domestik' && empty($kota)) {
        $_SESSION['error'] = "Kota harus diisi untuk wisatawan domestik!";
        header("Location: ../editpengunjung.php");
        exit;
    }

    if ($jenis_wisatawan == 'Mancanegara' && empty($negara)) {
        $_SESSION['error'] = "Negara harus diisi untuk wisatawan mancanegara!";
        header("Location: ../editpengunjung.php");
        exit;
    }
    ...
    if (!$stmt) {
        ...
        Swal.fire({
            icon: 'error',
            title: 'Gagal!',
            text: " . json_encode("Error preparing statement: " . $error) . "
        }).then(() => {
    ...
    if ($stmt->execute()) {
        ...
        Swal.fire({
            icon: 'success',
            title: 'Berhasil!',
            text: 'Data pengunjung berhasil diupdate'
        }).then(() => {
            window.location = '../datapengunjung.php';
        });
        </script>
        ...
        Swal.fire({
            icon: 'error',
            title: 'Gagal!',
            text: " . json_encode('Terjadi kesalahan: ' . $error) . "
        }).then(() => {
            window.location = '../datapengunjung.php';
        });
        </script>
