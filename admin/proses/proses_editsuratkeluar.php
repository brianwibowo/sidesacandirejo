...
if ($has_new_file_surat) {
    ...
    if ($_FILES['file_surat']['size'] > 10 * 1024 * 1024) {
        echo "<script>alert('Ukuran file surat melebihi 10 MB.'); window.history.back();</script>";
        exit;
    }
    ...
    if (!in_array($orig_ext, $allowed_surat_ext, true) || !in_array($real_mime, $allowed_surat_mime, true)) {
        echo "<script>alert('Format file tidak didukung. Gunakan PDF, gambar (JPG/PNG/WebP/GIF), atau Word (DOC/DOCX).'); window.history.back();</script>";
        exit;
    }
