...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .page-title p  { font-size: 13px; color: #6b8f7e; }

    /* ── CARD ── */
    .card { background: #fff; border: 1px solid #e5ede8; border-radius: 12px; overflow: visible; box-shadow: 0 1px 4px rgba(0,0,0,0.04); }
    ...
    .filter-bar {
      ...
      padding: 12px 16px; border-bottom: 1px solid #f0f5f2;
      flex-wrap: wrap;
    }
    ...
    .search-input {
      ...
      padding: 8px 14px 8px 34px; font-size: 13px; color: #1e3a2f;
    ...
    .filter-date {
      ...
      padding: 7px 10px; font-size: 13px; color: #1e3a2f;
      background: #fff; outline: none; cursor: pointer;
      transition: border-color 0.15s; font-family: inherit; min-width: 140px;
    }
    .filter-date:focus { border-color: #2e7d4f; }

    .filter-select {
      border: 1px solid #d6e6dc; border-radius: 8px;
      padding: 7.5px 10px; font-size: 13px; color: #1e3a2f;
      background: #fff; outline: none; cursor: pointer;
    ...
    .btn-filter {
      ...
      width: 34px; height: 34px; border: 1px solid #d6e6dc;
    ...
    .btn-reset-filter {
      ...
      padding: 7px 11px; font-size: 12.5px; font-weight: 500; color: #b03030;
    ...
    .btn-tidakhadir { background: #fef5e7; color: #c0742a; }
    .btn-tidakhadir:hover { background: #c0742a; color: #fff; }
    ...
    .btn-modal-cancel {
      ...
      border-radius: 7px; padding: 8px 15px; font-size: 12.5px; font-weight: 500;
      cursor: pointer; transition: all 0.15s; font-family: inherit;
    }
    .btn-modal-cancel:hover { background: #f4f7f5; color: #1e3a2f; }
    .btn-modal-submit {
      display: inline-flex; align-items: center; gap: 6px;
      background: #1e3a2f; color: #fff; border: none; border-radius: 7px;
      padding: 8px 18px; font-size: 12.5px; font-weight: 600; cursor: pointer;
      transition: background 0.15s; font-family: inherit;
    }
    .btn-modal-submit:hover { background: #2d5540; }
    .btn-modal-submit:disabled { opacity: 0.6; cursor: not-allowed; }

    .modal-spinner {
      width: 24px; height: 24px; border: 2.5px solid #d6e6dc;
      border-top-color: #2e7d4f; border-radius: 50%;
      animation: modalSpin 0.75s linear infinite; margin: 0 auto;
    }
  ...
  <div class="content-inner">
    <div class="card">
      ...
      <div class="card-header-table">
        <div>
          <h1>Semua Booking</h1>
          <p>Kelola seluruh data reservasi dan kunjungan wisatawan Desa Candirejo</p>
        </div>
      </div>
      ...
      <form method="GET" action="" id="filterForm">
        <div class="filter-bar">
          ...
          <!-- Show per page (Default 15 baris agar tidak perlu scroll vertikal) -->
          ...
          <?php if ($search !== '' || $filter !== '' || $tanggal !== ''): ?>
          <a href="booking_semua.php" class="btn-reset-filter" title="Reset Filter">
          ...
          <div style="margin-left:auto;">
            <a href="tambah_booking.php?ref=booking_semua.php" class="btn-tambah">
            ...
            <tr class="data-row">
              ...
              <td>
                <div class="btn-action-wrap">
                  <!-- Detail -->
                  <a href="detail_booking.php?id=<?php echo $row['id']; ?>&ref=booking_semua.php" class="btn-icon btn-detail" data-tooltip="Lihat Detail" title="Lihat Detail">
                  ...
                  <!-- Edit -->
                  <a href="edit_booking.php?id=<?php echo (int)$row['id']; ?>&ref=booking_semua.php" class="btn-icon btn-edit" data-tooltip="Edit Booking" title="Edit Booking">
                  ...
                  <button
                    type="button"
                    class="btn-icon btn-tidakhadir"
                    data-tooltip="Tandai Tidak Hadir"
                    title="Tandai Tidak Hadir"
                    onclick="konfirmasiAksi(<?php echo $row['id']; ?>, 'tidak_hadir', '<?php echo htmlspecialchars($row['nama'], ENT_QUOTES); ?>')">
                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>
                  </button>
      ...
      <div class="table-footer">
        <div>
          Menampilkan <strong><?php echo $total_data > 0 ? ($offset + 1) : 0; ?></strong> - <strong><?php echo min($offset + $per_page, $total_data); ?></strong> dari <strong><?php echo $total_data; ?></strong> data booking
        </div>
        ...
        <ul class="pagination">
          ...
          <?php
            // Rentang halaman yang ditampilkan
          ...
          <li class="page-item <?php echo $page >= $total_pages ? 'disabled' : ''; ?>">
            <a class="page-link" href="<?php echo getPageUrl($page + 1); ?>" title="Halaman Selanjutnya">&raquo;</a>
          </li>
        </ul>
        <?php endif; ?>
      </div>
    ...
    <form id="formModalBooking" onsubmit="handleFormSubmit(event)">
      ...
      <div class="modal-card-body" id="modalBody">
        ...
        <div id="modalLoading" style="display:none;text-align:center;padding:45px 20px;color:#2e7d4f;">
          ...
          <p style="margin-top:12px;font-size:13.5px;font-weight:500;color:#5a7d6d;">Memuat data booking...</p>
        </div>
          ...
          <div class="modal-sub-box" id="m_sub_makan_tour" style="display:none;">
            ...
            <div class="modal-radio-group">
              <label class="modal-radio-pill">
                ...
                <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                Without Lunch
              </label>
              <label class="modal-radio-pill">
                <input type="radio" name="opsi_makan_tour" value="with_lunch">
                <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path d="M18 8h1a4 4 0 010 8h-1M2 8h16v9a4 4 0 01-4 4H6a4 4 0 01-4-4V8z"/><line x1="6" y1="1" x2="6" y2="4"/><line x1="10" y1="1" x2="10" y2="4"/><line x1="14" y1="1" x2="14" y2="4"/></svg>
                With Lunch
              </label>
            </div>
          </div>
...
/* ══════════════════════════════════════════════
   MODAL POP-UP LOGIC (CREATE & UPDATE)
══════════════════════════════════════════════ */
...
// Highlight radio pill when clicked
...
function bukaModalTambah() {
  ...
  // Default Jenis Wisatawan: Domestik
  setJenisWisatawan('Domestik');

  // Hide package sub fields
  sembunyikanSemuaSubPaket();
...
function bukaModalEdit(id) {
  ...
  fetch('proses/get_booking.php?id=' + id)
    ...
    .then(res => {
      ...
      if (d.opsi_makan_tour) {
        setRadioVal('opsi_makan_tour', d.opsi_makan_tour);
      }
      if (d.jenis_makanan_paket) {
        setRadioVal('jenis_makanan_paket', d.jenis_makanan_paket);
      }
      if (d.opsi_cooking_lesson) {
        setRadioVal('opsi_cooking_lesson', d.opsi_cooking_lesson);
      }
      if (d.opsi_gamelan) {
        setRadioVal('opsi_gamelan', d.opsi_gamelan);
      }
...
(function() {
  ...
  if (msg === 'sukses_tambah') {
    Swal.fire({
      ...
      text: 'Data booking baru telah tersimpan ke dalam sistem.',
    ...
    Swal.fire({
      ...
      text: 'Perubahan data booking berhasil disimpan.',
      confirmButtonColor: '#2e7d4f'
    });
    ...
    Swal.fire({
      ...
      text: 'Data booking telah dihapus dari sistem.',
      confirmButtonColor: '#2e7d4f'
    });
  } else if (msg === 'sukses_checkin') {
    Swal.fire({
      icon: 'success',
      title: 'Check-in Berhasil!',
      text: 'Data telah tercatat dan dimasukkan ke buku pengunjung.',
      confirmButtonColor: '#2e7d4f'
    });
  } else if (msg === 'gagal') {
    Swal.fire({
      ...
      text: detail || 'Terjadi kesalahan sistem.',
      confirmButtonColor: '#c0392b'
    });
  }
