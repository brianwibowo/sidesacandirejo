...
$sql = "SELECT id, nama, pax, status, tanggal_kunjungan, pilihan_paket_wisata, agen_wisata
        FROM tb_booking
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .btn-checkin-panel {
      flex: 1; background: #1e3a2f; color: #fff; border: none;
      border-radius: 7px; padding: 8px 6px; font-size: 12.5px; font-weight: 500;
      cursor: pointer; transition: background 0.15s;
    }
    .btn-checkin-panel:hover { background: #2d5540; }

    /* Tombol Detail Booking di panel */
    .btn-detail-panel {
      flex: 1; display: flex; align-items: center; justify-content: center; gap: 5px;
      background: #f0f7f3; color: #1e3a2f;
      border: 1px solid #c4ddd0; border-radius: 7px;
      padding: 8px 6px; font-size: 12.5px; font-weight: 500;
      cursor: pointer; transition: background 0.15s;
      text-decoration: none;
    }
    .btn-detail-panel:hover { background: #dff0e8; border-color: #1e3a2f; }

    /* Tombol Tambah Booking di panel */
    .btn-tambah-booking {
      display: flex; align-items: center; justify-content: center; gap: 7px;
      width: 100%; background: #fff; color: #1e3a2f;
      border: 1.5px dashed #a8c8b8; border-radius: 8px;
      padding: 10px; font-size: 13px; font-weight: 500;
      cursor: pointer; transition: background 0.15s, border-color 0.15s;
      margin-top: 14px; text-decoration: none;
    }
    .btn-tambah-booking:hover { background: #f0f7f3; border-color: #1e3a2f; color: #1e3a2f; }
    .btn-tambah-booking svg  { flex-shrink: 0; }
      ...
      <div class="side-panel" id="sidePanel">
        <?php if (!$selected_date || empty($selected_bookings)): ?>
          <?php if ($selected_date && empty($selected_bookings)): ?>
            ...
            <a href="tambah_booking.php?tanggal=<?php echo htmlspecialchars($selected_date); ?>" class="btn-tambah-booking">
          ...
          <?php else: ?>
            ...
            <a href="tambah_booking.php" class="btn-tambah-booking">
              <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path d="M12 5v14M5 12h14"/></svg>
              Tambah Booking
            </a>
          <?php endif; ?>
        ...
        <?php else: ?>
          ...
          <?php foreach ($selected_bookings as $b):
            ...
            $agen = (!empty($b['agen_wisata']) && strtolower(trim($b['agen_wisata'])) !== 'null')
                    ? trim($b['agen_wisata'])
                    : trim($b['nama']);
          ?>
          <div class="booking-item">
            <div class="booking-item-top">
              <span class="booking-item-nama"><?php echo htmlspecialchars($agen); ?></span>
            ...
            <?php endif; ?>
            <a href="detail_booking.php?id=<?php echo (int)$b['id']; ?>&ref=booking_kalender.php" class="btn-detail-panel">
              <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
              Lihat Detail
            </a>
            </div>
          </div>
