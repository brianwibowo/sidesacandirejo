...
function redir_tambah($msg, $detail = '') {
    // Simpan semua input POST ke session supaya form bisa di-prefill
    $_SESSION['form_tambah'] = $_POST;
    $q = '?msg=' . urlencode($msg);
    if ($detail) $q .= '&detail=' . urlencode($detail);
    header("Location: ../tambah_booking.php$q");
    exit;
}
...
$keterangan  = san($_POST['keterangan'] ?? '', 500);
...
if (!$tanggal || !preg_match('/^\d{4}-\d{2}-\d{2}$/', $tanggal))
    redir_tambah('gagal', 'Tanggal kunjungan tidak valid.');
if (!$paket)   redir_tambah('gagal', 'Paket wisata harus dipilih.');
if (!$nama)    redir_tambah('gagal', 'Nama pengunjung tidak boleh kosong.');
if (!$jenis_w) redir_tambah('gagal', 'Jenis wisatawan harus dipilih.');
    ...
    $sql = "
        INSERT INTO tb_booking
            (tanggal_kunjungan, pilihan_paket_wisata, opsi_makan_tour, jenis_makanan_paket,
             ...
             nama, pax, agen_wisata, driver_agent_guide, local_guide, keterangan, status, created_at)
        VALUES (
            '$tanggal', '$paket',
            ...
            " . sqlStr($keterangan) . ", 'pending', NOW()
        )
    ";
    if (mysqli_query($db, $sql)) {
        redir('../booking_dashboard.php', 'sukses_tambah');
    } else {
        redir_tambah('gagal', 'DB error: ' . mysqli_error($db));
    }
}
...
elseif ($aksi === 'edit') {
    ...
    $allowed_ref = [
        'booking_dashboard.php', 'booking_semua.php', 'booking_pending.php',
        'booking_checkin.php', 'booking_tidakdatang.php', 'booking_kalender.php',
        'booking_laporan.php'
    ];
    $ref_raw = basename($_POST['ref'] ?? 'booking_dashboard.php');
    $ref = in_array($ref_raw, $allowed_ref, true) ? $ref_raw : 'booking_dashboard.php';
    ...
    $sql = "
        UPDATE tb_booking SET
            ...
            local_guide          = '$lg',
            keterangan           = " . sqlStr($keterangan) . "
        WHERE id = $id AND status = 'pending'
    ";
    if (mysqli_query($db, $sql)) {
        redir("../$ref", 'sukses_edit');
    } else {
        redir("../edit_booking.php?id=$id&ref=$ref", 'gagal', 'DB error: ' . mysqli_error($db));
    }
}
